...
(function(h, g) {
                ...
                if (!j) {
                    ...
                    h(k).bind("click.YouTubePopup",
                        function() {
                            ...
                            var n = g.location.protocol +
                                "//www.youtube.com/embed/" +
                                o +
                                "?rel=0&showsearch=0&autohide=" +
                                i.autohide;
                            n += "&autoplay=" + i.autoplay +
                                "&controls=" + i.controls +
                                "&fs=" + i.fs +
                                "&loop=" + i.loop;
                            n += "&showinfo=" + i.showinfo +
                                "&color=" + i.color +
                                "&theme=" + i.theme;
                            ...
                            b.dialog("open");
                            h(".ui-widget-overlay").fadeTo(
                                "fast", i.overlayOpacity
                            );
                            var l = h(
                                ".ui-dialog-titlebar"
                            );
    ...
    function e(j, l, i) {
        var k =
            '<iframe title="YouTube video player" style="margin:0; padding:0;" width="' +
            l + '" ';
        k += 'height="' + i + '" src="' + j +
    ...
    function a(j) {
        var i = "https://gdata.youtube.com/feeds/api/videos/" + j +
            "?v=2&alt=json";
        h.ajax({
            url: i,
            dataType: "jsonp",
    ...
    h.fn.YouTubePopup = function(i) {
        if (d[i]) {
            return d[i].apply(this, Array.prototype.slice.call(
                arguments, 1))
        } else {
            if (typeof i === "object" || !i) {
                ...
                h.error("Method " + i +
                    " does not exist on jQuery.YouTubePopup")
            }
        }
    };
